export const QuizData = [
    {
        question: "What is React.js?",
        options:[ "1. Open-source JavaScript back-end library", 
        "2. JavaScript front-end library to create a database",
         "3. Free and open-source JavaScript front-end library", 
         "4. None of the mentioned"],
        answer: 3
    },
    {
        question: "Which of the following acts as the input of a class-based component?",
        options:["1. Class", "2. Props", "3. Factory", "4. None of the mentioned"],
        answer: 2
    },
    {
        question: "React.js is written in which of the following language?",
        options: ["1. C", "2. C++", "3. JavaScript", "4. Java"],
        answer: 3
    },
    {
        question: "How many elements can a valid react component return?",
        options: ["1. React doesn't return element", "2. One Element", "3. More than 1 element", "4.  None of the mentioned"],
        answer:2
    },{
        question: " Which of the following command is used to Install create-react-app?",
        options: ["1. npm install create-react-app",
         "2. npm install -f create-react-app",
          "3. npm install -g create-react-app", 
          "4. install -g create-react-app"],
        answer:3
    },{
        question: "Which of the following is method is not a part of ReactDOM?",
        options: ["1. ReactDOM.hydrate()", "2. ReactDOM.destroy()", "3. ReactDOM.createPortal()", 
        "4. All of the mentioned"],
        answer:2
    },
    {
        question: "Which of the following is correct about prop in react?",
        options: ["1. Can be changed inside another component", 
        "2. Can be changed inside the component", 
        "3. Cannot be changed in the component", 
        "4. All of the mentioned"],
        answer:3
    },{
        question: "Which of the following is used to pass data to a component from outside in React?",
        options: ["1. props", "2. render with arguments", "3. setState", "4. PropTypes"],
        answer:1
    }
    ,{
        question: "How many ways of defining your variables in ES6?",
        options: ["1. 1", "2. 3", "3. 4", "4. 5"],
        answer:2
    }
    ,{
        question: "What year was JavaScript launched?",
        options: ["1. 1996", "2. 1995", "3. 1994", "4. none of the above"],
        answer:2
    }
];